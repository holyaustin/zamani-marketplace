...
export default function CollectionView({ light, vibrant, dark }) {
  ...
  const blockchain_id = id || CONFIG.TEMPLATE.block_chain_id;
  const address_id = address || CONFIG.TEMPLATE.collection_address;
  ...
  const handleGraph = async (filter) => {
    ...
    setGraph([]);
    setWei([]);
    const from = moment().subtract(filter, "days").format("YYYY-MM-DD");

    // If filter is 0 (All time), apply different parameters
    const api_call = filter > 0 ?
      // 2 dates (from - to)
  ...
  return (
    <>
      ...
      <div className="main">
        {!address ? (
          <div className="global" style={{ color: light || "#FF4C8B" }} onClick={() => { navigate("/global"); }}>
            Global View
            <Icon icon="chevron-right" size={24} intent="primary" color={light || "#FF4C8B"} className="icon" />
          </div>
        )
          : (
          ...
          <div className="info">
              ...
              {activeLoader ?
                ...
                : (
                  <img
                    ...
                    src={collectionData[0]?.first_nft_image_256}
            ...
            <div className="details">
              <div className="title-cont">
                ...
                >{address_id} <Icon icon="share" size={15} intent="primary" color={light || "#FF4C8B"} className="share" />
                </h3>
        ...
        <div className="bottom-section">
          ...
          {activeLoader ? (
            <div className="collection-load">
              <img src={Loader} alt="No preview available" />
            </div>
          )
            : (
              <div className="collection-display">
                <div className="nft" onClick={() => { navigate(`/nft/${collectionData[0]?.collection_address}/${collectionData[0]?.first_nft_image_token_id}/${collectionData[0].chain_id}`); }}>
                  <img src={collectionData[0]?.first_nft_image_256} alt="No preview available" />
                </div>
                <div className="nft" onClick={() => { navigate(`/nft/${collectionData[0]?.collection_address}/${collectionData[0]?.second_nft_image_token_id}/${collectionData[0].chain_id}`); }}>
                  <img src={collectionData[0]?.second_nft_image_256} alt="No preview available" />
                </div>
                <div className="nft" onClick={() => { navigate(`/nft/${collectionData[0]?.collection_address}/${collectionData[0]?.third_nft_image_token_id}/${collectionData[0].chain_id}`); }}>
                  <img src={collectionData[0]?.third_nft_image_256} alt="No preview available" />
                </div>
                <div className="nft" onClick={() => { navigate(`/nft/${collectionData[0]?.collection_address}/${collectionData[0]?.fourth_nft_image_token_id}/${collectionData[0].chain_id}`); }}>
                  <img src={collectionData[0]?.fourth_nft_image_256} alt="No preview available" />
                </div>
                <div className="nft" onClick={() => { navigate(`/nft/${collectionData[0]?.collection_address}/${collectionData[0]?.fifth_nft_image_token_id}/${collectionData[0].chain_id}`); }}>
